...
function calculateCount() {
  ...
  availableCount.innerHTML = `<p class="text-[#64748B]">${availableCount.innerText = allcards.children.length} Jobs</p>`
  ...
  if (currentStatus === "all-toggle-btn") {
    ...
    availableCount.innerHTML = `<p class="text-[#64748B]"> ${availableCount.innerText = interviewList.length} of ${availableCount.innerText = allcards.children.length} Jobs</p>`;
  } 
  else if (currentStatus === "reject-toggle-btn") {
    availableCount.innerHTML = `<p class="text-[#64748B]"> ${availableCount.innerText = rejectedList.length} of ${availableCount.innerText = allcards.children.length} Jobs</p>`;
  }
}
...
mainContainer.addEventListener("click", function (event) {
  if (event.target.classList.contains("interview-btn")) {
    ...
    const companyExist = interviewList.find(
      (item) => item.companyName.trim() == cardInfo.companyName.trim(),
    );
...
function interviewRender() {
  ...
  for (let interview of interviewList) {
    ...
    div.className =
      "bg-white rounded-[8px] p-[24px] space-y-[16px] flex justify-between";
    div.innerHTML = `
